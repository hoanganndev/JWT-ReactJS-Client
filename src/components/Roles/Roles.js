...
import "./Roles.scss";
import { createRoles_Client } from "../../services/roleService_Client";
...
const Roles = () => {
    ...
    const handleSave = async () => {
        let invalidObject = Object.entries(listChilds).find(
            ([key, child], index) => {
                return child && !child.url; // Return child when url=null
            }
        );
        if (!invalidObject) {
            let data = buildDataToPersist();
            let res = await createRoles_Client(data);
                    ...
                    <div className="role-parent">
                        {Object.entries(listChilds).map(
                            ([key, child], index) => {
                                return (
                                    <div
                                        className={`row role-child ${key} mt-2`}
                                        key={`child-${key}`}
                                    >
