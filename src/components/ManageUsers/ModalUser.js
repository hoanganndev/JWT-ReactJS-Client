...
const ModalUser = props => {
    ...
    const checkValidateInputs = () => {
        // If the user update, I don't need to check the validate Inputs
    ...
    const handleConfirmUser = async () => {
        ...
        let isValidate = checkValidateInputs();
        if (isValidate) {
            let res =
                action === "CREATE"
                    ? await createNewUser_Client({
                          ...userData,
                          groupId: userData["group"],
                      })
                    : await updateCurrentUser_Client({
                          ...userData,
                          groupId: userData["group"] ? userData["group"] : 3, // 3 is a customer
                      });
            if (res && res.errorCode === 0) {
                setUserData({
                    ...defautlUserData,
